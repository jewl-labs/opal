...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        'group/select border-border bg-muted hover:bg-accent hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground hover:border-muted-foreground/70 focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-popup-open:border-ring/70 data-popup-open:bg-muted/45 data-placeholder:text-muted-foreground dark:bg-input/20 dark:hover:bg-input/45 dark:data-popup-open:bg-input/55 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 flex w-fit items-center justify-between gap-2 border py-2 pr-2.5 pl-2.5 text-sm font-medium whitespace-nowrap shadow-xs transition-[border-color,box-shadow,background-color,color] outline-none select-none focus-visible:ring-3 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:ring-3 data-[size=default]:h-8 data-[size=sm]:h-7 data-[size=sm]:rounded-[min(var(--radius-md),10px)] *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5',
        className
      )}
      {...props}
    >
...
function SelectLabel({ className, ...props }: SelectPrimitive.GroupLabel.Props) {
  return (
    <SelectPrimitive.GroupLabel
      ...
      className={cn(
        'text-muted-foreground px-2 py-1 text-[11px] font-semibold tracking-wide uppercase',
        className
      )}
      {...props}
    />
