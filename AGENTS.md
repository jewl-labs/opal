...
**Program ID (localnet):** `72kZgK51BsRWaLVcMriBuskWbn4d5E4P9HVeV3oBFp2y`
...
# Regenerate TypeScript client from IDL using Codama
# This will be used on the frontend, the tests continue to use web3.js
# bun run gen:client
```

**Build order:** `anchor build` must run first — it writes `target/idl/opal.json`, which `gen:client` reads. The generated client lands in `web/client/src/generated/`.

> The IDL and `target/types/opal.ts` generated by `anchor build` are sufficient for running tests.
