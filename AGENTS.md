...
**Build order:** `anchor build` writes `target/idl/opal.json` and `target/types/opal.ts`. Tests and any TypeScript clients use `@coral-xyz/anchor` with `@solana/web3.js` against those artifacts (see `tests/opal.test.ts`).
