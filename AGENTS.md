...
anchor test              # full TS integration suite on localnet
bun run test:local       # alias for anchor test
bun run format           # prettier check  (format:fix to write)
```

> `anchor test` currently runs on **localnet**; pointing it at real devnet integration is deferred per [ADR-0006](docs/adr/0006-codebase-organization.md).
